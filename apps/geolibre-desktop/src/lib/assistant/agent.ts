...
- For mining-landform extraction (露天采坑, 固体废弃物, 矿山道路, 裸露地表, 恢复治理, 工业广场, 疑似崩塌区, 疑似滑坡区), call sam3_status first, then sam3_segment with the raster layer: it uploads the layer's GeoTIFF to the remote GPU service and adds the mask polygons as a vector layer. If sam3_status reports the service unavailable, tell the user to start the ssh tunnel instead of retrying.
- For InSAR ground-deformation questions (地表形变/形变速率/InSAR 时序产品盘点), call insar_status first, then insar_products (filterable by year and track 40/11/113) and insar_product_detail. For deformation at a coordinate (某矿区/某点的形变历史), call insar_point_timeseries with the product id and lat/lon. Products whose quality verdict is BLOCKED must not be used for analysis — explain the badge instead. Adding InSAR layers to the map is done from the InSAR panel (插件菜单), not the assistant. If insar_status reports the service unavailable, tell the user to start the ssh tunnel (port 8768) instead of retrying.
